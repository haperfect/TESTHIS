...
public class TestLuongDichVu_13792 extends TiepNhanBenhNhan {
	...
	public String NguoiLienHe, SoDienThoaiNguoiLienHe, DoiTuong, UuTien, HinhThuc, LiDo, TenDichVu, ThuTienSau,
			NoiThucHien;
	public String soTiepNhan, tenThuoc;
	...
	// BS01 ket luan kham : Dieu tri ngoai Tru
	@Test(priority = 6)
	public void DichVu_13792_6() {
		clickOn(FormKhamBenh.FormKhamBenh_KetLuan);
		moveMouseHorizontallyFromLogo(FormKhamBenh.FormKhamBenh_HuongDieuTri, 296);
		s.click();
		// Chuyển viện, nhập đơn thuốc
		sleep(2);
		clickOn(FormKhamBenh.HuongDieuTri_ChuyenVien);
		sleep(2);

		// chuyen den benh vien
		clickToaDo(567, 311);
		waitForObjectPresent(FormKhamBenh.ChuyenDen_BenhVienThanhNhan, 4);
		clickOn(FormKhamBenh.ChuyenDen_BenhVienThanhNhan);
		...
		// Click nut Thong tin chuyen vien
		clickOn(FormKhamBenh.Nut_ThongTinChuyenVien);
		// dien thong tin vao form thong tin chuyen vien
		// dau hieu lam sang
		setClipboardValue("tim đập nhanh, mạch thấp");
		s.click();
		s.type("v", Key.CTRL);
		s.type(Key.TAB);

		moveMouseHorizontallyFromLogo(FormKhamBenh.ThongTinChuyenVien_ChuanDoan, 100);
		s.click();
		setClipboardValue("Bệnh Tim");

		s.click();
		s.type("v", Key.CTRL);
		s.type(Key.TAB);
		// click nút Lưu
		clickOn(FormKhamBenh.ThongTinChuyenVien_Luu);
		// Chon kho thuoc
		sleep(2);
		clickToaDo(568, 342);
		waitForObjectPresent(FormKhamBenh.KhoThuoc_KhoChinh, 3);
		clickOn(FormKhamBenh.KhoThuoc_KhoChinh);
		// nhap don thuoc
		sleep(1);
		clickToaDo(470, 451);
		sleep(1);
		...
		// Check in giay chuyen tuyen
		clickOn(FormKhamBenh.FormKhamBenh_NutIn);
		sleep(1);
		s.click();
		if (waitForObjectPresent(FormKhamBenh.FormKhamBenh_GiayChuyenTuyen, 3)) {
			s.type(Key.F4, Key.ALT);
			setTestcaseStatus("PASS", "Đã in giấy chuyển tuyến thành công !");
		} else {
			setTestcaseStatus("FAIL", "Không thể in giấy chuyển tuyến thành công !");
		}
		...
		if (waitForObjectPresent(FormKhamBenh.FormKhamBenh_GiayKhamBenh, 3)) {
			s.type(Key.F4, Key.ALT);
			setTestcaseStatus("PASS", "Đã in đơn thuốc thành công !");
		} else {
			setTestcaseStatus("FAIL", "Không thể in đơn thuốc thành công  !");
