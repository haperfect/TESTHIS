...
public class TestLuongDichVu_13773 extends TiepNhanBenhNhan {
	...
	public String NguoiLienHe, SoDienThoaiNguoiLienHe, DoiTuong, UuTien, HinhThuc, LiDo, TenDichVu, ThuTienSau,
			NoiThucHien;
	public String soTiepNhan, noiDungBenhChinh, tenThuoc;
	...
	// BS01 ket luan kham : chọn kham them, 
	@Test(priority = 6)
	public void DichVu_13773_6() {
		clickOn(FormKhamBenh.FormKhamBenh_KetLuan);
		moveMouseHorizontallyFromLogo(FormKhamBenh.FormKhamBenh_HuongDieuTri, 302);
		s.click();
		
		// Chon kham them
		waitForObjectPresent(FormKhamBenh.HuongDieuTri_KhamThem, 4);
		clickOn(FormKhamBenh.HuongDieuTri_KhamThem);
		
		// Chon dich vu kham them
		clickToaDo(567, 310);
		waitForObjectPresent(FormKhamBenh.DichVuKham_KhamNoi01, 2);
		clickOn(FormKhamBenh.DichVuKham_KhamNoi01);
		...
		if (waitForObjectPresent(HisActions.HIS_LOI_UNGDUNG, 2)) {
			...
			waitForObjectPresent(FormKhamBenh.FormKhamBenh_NutInGiayKhamThem, 5);
			clickOn(FormKhamBenh.FormKhamBenh_NutInGiayKhamThem);

			if (waitForObjectPresent(FormKhamBenh.FormKhamBenh_GiayChungNhanNghiOm, 4)) {
				s.type(Key.F4, Key.ALT);
				setTestcaseStatus("PASS", "Đã in thành công giấy khám thêm cho bệnh nhân");
			} else {
				setTestcaseStatus("FAIL", "Không in thành công giấy khám thêm cho bệnh nhân !");
